  ...
  useEffect(() => {
    if (characterHealth === 0 || enemyHealth === 0) {
      (async () => {
        await pause(1000);
        onGameEnd(
          characterHealth === 0 ? bossEncounter.enemy : selectedCharacter,
        );
      })();
    }
    ...
    <>
      <div className={styles.screen}>
        ...
        <div className={styles.enemy}>
          <Enemy enemy={bossEncounter.enemy} health={enemyHealth} />
        </div>

        {/* Both characters */}
        <div className={styles.animation}>
          <BattleAnimation
            characterImage={characterAnimation}
            enemyImage={enemyAnimation}
          />
        </div>
        ...
        <div className={styles.menu}>
          {!inEncounter && turn === 0 && (
            <div>
              <BattleMenu
                onAttack={() =>
                  setEncounter({ battleMode: 'attack', turn })
                }
                onPotion={() =>
                  setEncounter({ battleMode: 'usePotion', turn })
                }
              />
            </div>
          )}
